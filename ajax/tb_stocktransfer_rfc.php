...
$aColumns = array('transferno','toloc','qty','createdby','createdon','action');
...
$sWhere = " where u.id = s.createdby and s.status = $status and s.inactive = 0 $addWhere";



$sQuery = "select SQL_CALC_FOUND_ROWS s.id,s.transferno, s.tot_qty, (select upper(dispname) from it_rfc_master where id = s.to_location_id) as toloc,s.tot_qty,u.name,s.transferdate,s.createtime from it_stock_transfer s,it_users u
	$sWhere 
	$sOrder
	$sLimit
";
             ...
             if ($aColumns[$i] == 'transferno') {
                ...
                if($status == StockTransferStatus::BeingCreated){
                    if($usertype == UserType::RFC){
                        ...
                        $row[] = '<button class="btn btn-primary" type="button" onclick="showPODetails(' . $obj->id . ')">View</button>';
                    }
//                           . '<button class="btn btn-primary" type="button" onclick="deleteGRN(' . $obj->id . ')">Delete</button>';            
                    ...
                    if($usertype == UserType::PurchaseOfficer){
                        ...
                        $row[] = '<button class="btn btn-primary" type="button" onclick="showPODetails(' . $obj->id . ')">View</button>
                             <br/><br/><button class="btn btn-primary" type="button" onclick="createChallan(' . $obj->id . ')">Create Challan</button><br/>';
                    }else{
                        $row[] = '<button class="btn btn-primary" type="button" onclick="showPODetails(' . $obj->id . ')">View</button>';
                    }
