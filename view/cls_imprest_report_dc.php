...
class cls_imprest_report_dc extends cls_renderer {
...
//            $objdc = $dbl->getCRDetailsByUserId($this->currStore->id);
//
//            $this->crid = $objcr->id;
//        } 
        else {
    ...
    function extraHeaders() {
        ...
        function updateDCSel(dcid){ 
            var dtrng = $("#reservation").val();
            window.location.href = "imprest/report/dc/dcid="+dcid+"&dtrng="+dtrng; 
        }
        ...
        function genExcelRep(){
            ...
            if(dccode == 0){
                ...
                window.location.href="formpost/genImprestExcel_dc.php?dcid="+dccode+"&dtrng="+dtrng;;
        ...
        //    alert("hiiii");
            var dtrng = $("#reservation").val();
            var url = "ajax/tb_imprestreport_dc.php?dcid="+<?php echo $this->dcid; ?>+"&uploaddate="+dtrng;   
...
//                         alert(impDetailsId);
                        ...
                        myWindow.location.href = 'ajax/printImprestVoucherDC.php?impDetailsId='+impDetailsId;                       
                        myWindow.focus();     
            }
                ...
                <?php if ($this->currStore->usertype != UserType::RFC) { ?>
                    <div class="col-md-3">
                        <select id="dccode" name="dccode" class=" form-control" data-show-subtext="true"
                                data-live-search="true" onchange="updateDCSel(this.value);" >
                            <option value="-1">Select Distribution Center</option>
                            <?php
                            foreach ($obj_dc_master as $dcmaster) {
                                $selected = "";
                                if ($dcmaster->id == $this->dcid) {
                                    $selected = "selected";
                                }
                                ?>
