...
try {
    ...
    if (isset($_SERVER['PHP_AUTH_USER']) && $_SERVER['PHP_AUTH_USER'] == 'shradhatally' && $_SERVER['PHP_AUTH_PW'] == 'intouch@2k18') {
        ...
        if (count($error) == 0) {
            ...
            $customer_obj = $dbl->getCRMastersByDate($datetime);
            if ($customer_obj) {
                $envelope = new SimpleXMLElement('<ENVELOPE/>');
                $name = "CRMaster_" . $datetime . "xml";
                ...
                $reqdesc->addChild("REPORTNAME", "Consignment Retailer Masters");
		...
		$log  = "User: ".$_SERVER['REMOTE_ADDR'].' - '.date("F j, Y, g:i a").PHP_EOL.
                ...
                file_put_contents('./log/crmasterlive_'.$local->format("Y.m.d_H:i:s.u").'.log', $log, FILE_APPEND);
            } else {
