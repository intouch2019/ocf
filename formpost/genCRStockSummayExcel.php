...
// Create a first sheet, representing points details data
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('J1', 'Qty (MT.)');
    $objPHPExcel->getActiveSheet()->setCellValue('K1', 'Price (Rs./MT)');
    $objPHPExcel->getActiveSheet()->setCellValue('L1', 'Value (Rs.)');
    $objPHPExcel->getActiveSheet()->setCellValue('M1', 'Stock Date');
...
//        'color' => array('rgb' => 'FF0000'),
    ...
    $objPHPExcel->getActiveSheet()->getStyle('J')->getNumberFormat()->setFormatCode('0.0000');
...
//c.name as ctg,p.name,p.desc1,p.desc2,p.thickness,p.hsncode,p.stdlength,s.batchcode,s.qty,s.createtime
    if ($date) {
        ...
        $result = $dbl->getCRStockSummeryByCloseDate($crid, $closeDate);
    } else {
        $closeDate = date("Y-m-d");
        $result = $dbl->getCRStockSummery($crid);
    }
//echo $result;
//return;
    $sr_no = 1;
    if ($result != null && isset($result)) {
        foreach ($result as $obj) {
            $priceObj = $dbl->getLatestProductPrice($crid, $obj->prodid, $closeDate);
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $rowCount, $obj->length);
            $roundQty = round($obj->qty, 4, PHP_ROUND_HALF_UP);
            $roundQty = sprintf("%.4f", $roundQty);
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $rowCount, $roundQty);
            if(isset($priceObj) && $priceObj != null){
                $prodPrice = $priceObj->price;
            }else{
                $prodPrice = 0;
            }
            $roundPrice = round($prodPrice, 2);
            $roundPrice = sprintf("%.2f", $roundPrice);
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $rowCount, $roundPrice);
            $roundValue = $roundPrice * $roundQty;
...
//        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $rowCount, $obj->createtime);
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12, $rowCount, $stockDate);
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $rowCount, "Data not available.");
        $stockDate = $closeDate;
    }

    // Redirect output to a client’s web browser (Excel5)
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="CR Stock Report_' . $stockDate . '.xls"');
